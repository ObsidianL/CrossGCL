...
if __name__ == '__main__':
    med_file = './mimic/PRESCRIPTIONS.csv'
    diag_file = './mimic/DIAGNOSES_ICD.csv'
    procedure_file = './mimic/PROCEDURES_ICD.csv'
    ...
    # for diagnosis
    diag_pd = diag_process(diag_file)#shape = 625434,3
    ...
    # ddi_matrix
    #映射，由于diag med pro的编号都是不连续的字符串，对其进行映射，并将映射文件保存为voc_final.pkl
    diag_voc, med_voc, pro_voc = create_str_token_mapping(data)
    records = create_patient_record(data, diag_voc, med_voc, pro_voc)   # diag,proc,medication按顺序存储，保存到records_final.pkl
